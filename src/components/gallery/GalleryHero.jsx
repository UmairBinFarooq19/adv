...
// GalleryHero — the immersive header for the Gallery. Same brand treatment as the
// Adventures/Destinations heroes (pine gradient, saffron eyebrow, ContourDivider,
// blur-up Media, shared Breadcrumbs), with optional counts as meta chips.
// `children` is a CTA slot.
export default function GalleryHero({ eyebrow, title, subtitle, image, breadcrumbs, photoCount, videoCount, children }) {
  return (
        ...
        <motion.div initial="hidden" animate="show" variants={stagger} className="max-w-3xl">
          ...
          <motion.div variants={fadeUp} className="mt-6 flex flex-wrap gap-2.5">
            <span className="inline-flex items-center gap-1.5 rounded-full bg-white/10 px-3.5 py-1.5 text-sm font-medium backdrop-blur">
              <Camera className="h-4 w-4 text-saffron-300" aria-hidden="true" /> {photoCount} photographs
            </span>
            <span className="inline-flex items-center gap-1.5 rounded-full bg-white/10 px-3.5 py-1.5 text-sm font-medium backdrop-blur">
              <Images className="h-4 w-4 text-saffron-300" aria-hidden="true" /> {videoCount} films
            </span>
          </motion.div>
